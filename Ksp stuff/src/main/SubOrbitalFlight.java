...
public class SubOrbitalFlight {
	public static void main(final String[] args) throws IOException, RPCException, StreamException, InterruptedException {
...

		{
			...
			Expression expr = Expression.lessThan(connection, Expression.call(connection, solidFuel), Expression.constantFloat(connection, 0.1f));
...

		{
			...
			Expression expr = Expression.greaterThan(connection, Expression.call(connection, meanAltitude), Expression.constantDouble(connection, 10000));
			Event event = krpc.addEvent(expr);
			synchronized (event.getCondition()) {
				event.waitFor();
			}
...

		{
			...
			Expression expr = Expression.greaterThan(connection, Expression.call(connection, apoapsisAltitude), Expression.constantDouble(connection, 100000));
			Event event = krpc.addEvent(expr);
			synchronized (event.getCondition()) {
				event.waitFor();
			}
		...
		{
			ProcedureCall srfAltitude = connection.getCall(vessel.flight(null), "getSurfaceAltitude");
			Expression expr = Expression.lessThan(connection, Expression.call(connection, srfAltitude), Expression.constantDouble(connection, 1000));
			Event event = krpc.addEvent(expr);
			synchronized (event.getCondition()) {
				event.waitFor();
			}
